...
We evaluated **Agent Skill Security Review V2.1** with **GPT-5.6 Sol** in a fresh-thread evaluation against the 99-case adversarial corpus before exposing the answer key. V2.1 + GPT-5.6 Sol produced **81/99 exact benign/review/dangerous classifications** and **95/99 correct benign-vs-risky classifications**. Separately, the 99-case corpus began as greenfield material for Commonset's deterministic scanner and later became a diagnostic regression set after we used what those cases exposed to improve the implementation.

The deterministic scanner now produces **89/99 exact** and **98/99 benign-vs-risky** classifications on that corpus, with **0 benign false positives**, while the separate **151-case frozen regression suite remains 151/151**. Because those 99 cases informed scanner development, the current deterministic result is a regression/diagnostic measurement rather than a blind estimate of future performance. Model-driven Skill results may also vary by model, version, execution environment, and context.

We are not publishing a numeric result for the scanner's original pass over the 99-case corpus until we reconcile conflicting historical baseline records and can tie the number to a specific scanner revision, corpus revision, and scoring procedure. The current diagnostic and frozen regression results above have independent provenance.
