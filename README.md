...
We evaluated **Agent Skill Security Review V2.1** with **GPT-5.6 Sol** in a fresh-thread evaluation against the 99-case adversarial corpus before exposing the answer key. V2.1 + GPT-5.6 Sol produced **81/99 exact benign/review/dangerous classifications** and **95/99 correct benign-vs-risky classifications**. Separately, the deterministic scanner's initial pass over the greenfield 99-case corpus produced **37/99 exact** and **57/99 benign-vs-risky** classifications before we used the corpus to improve the scanner.
...
The `67/99` figure documented during scanner development is not a conflicting baseline. It was an intermediate result reached after improvement work had already begun and before the final risk-composition step that raised the scanner to 89/99. The methodology document records that chronology explicitly.
